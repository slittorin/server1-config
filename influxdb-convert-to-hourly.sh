...
_usage() {
    ...
    echo "Files will be named 'influx-import-NEWENTITYID-YYYY-MM-DD.csv"
    echo ""
...
_iterate() {
    ...
    do
        ...
        import_file="${import_entity_dir}/influx-import-${options_entity_id_to}-${date_export}.csv"
...
# Therefore we utilize command line instead.
    ...
    if [ ${RESULT_CODE} -ne 0 ]; then
        ...
        if [ ${number_rows} -gt 4 ]; then
            headers_found=$(cat ${csv_export_file} | grep "${header_row}" | wc -l | awk '{ print $1 }')
...
_import_file_setup() {
    ...
    for LOOP in $(seq ${csv_number_of_split_files}); do
       # Find where the headers starts.
       header_starts=$(grep -wn "${header_row}" "export-split-${LOOP}.csv" | cut -d: -f1)

       head -${header_starts} "export-split-${LOOP}.csv" > "import-split-${LOOP}.csv"
    done
    ...
    if [ ${csv_files_with_value} -ne 1 ]; then
        ...
        # ,,4,2023-01-01T00:00:00Z,2023-01-01T23:59:59.999999Z,2023-01-01T00:01:02.044883Z,25925.357,value,kWh,sensor,metering_total_absorbed
        # ID 1: Empty
        # ID 2: 4
        # ID 3: 2023-01-01T00:00:00Z
        # ID 4: 2023-01-01T23:59:59.999999Z
        # ID 5: 2023-01-01T00:01:02.044883Z
        # ID 6: 25925.357
        # ID 7: value
        # ID 8: kWh
        # ID 9: sensor
        # ID 10: metering_total_absorbed
        column_datetime=5 		# HARDCODED.
        column_value=6 			# HARDCODED.
        column_entity_id=${columns} 	# The entity_id is always last.
...
_import_file_iterate() {
    ...
    # Find where the headers starts.
    header_starts=$(grep -wn "${header_row}" "export-split-${csv_file_with_value}.csv" | cut -d: -f1)

    # We only start if the file has more lines than header.
    rows_total=$(wc -l < "export-split-${csv_file_with_value}.csv")
    if [ ${rows_total} -gt ${header_starts} ]; then
        ...
        do
            ...
            if [ ${row_current} -gt ${header_starts} ]; then
                # Split into array.
                IFS=','
                ...
                # We check if we directly are after header.
                [ ${row_current} -eq $((${header_starts}+1)) ] && check_after_header=1 || check_after_header=0
...
#echo "${row_current} ${line}" 
#echo "Hour: ${hour_current}"
#echo "header_starts: ${header_starts}"
#echo "Check_after_header: ${check_after_header}"
#echo "Check_last_row: ${check_last_row}"
#echo "Check_hour_change: ${check_hour_change}"

                # If we are after header, and last row. then there is only one result.
                if [ ${check_after_header} -eq 1 ] && [ ${check_last_row} -eq 1 ]; then
                    if [ ${debug} -gt 0 ]; then
                       echo "$(date +%Y%m%d_%H%M%S):     At ${row_current} of ${rows_total}: Result after header, and last row."
                    fi
                ...
                # If we are after header, and not last row. then we start fresh.
                if [ ${check_after_header} -eq 1 ] && [ ${check_last_row} -eq 0 ]; then
                    if [ ${debug} -gt 0 ]; then
                       echo "$(date +%Y%m%d_%H%M%S):     At ${row_current} of ${rows_total}: After header, and not last row."
                    fi
                ...
                if [ ${check_after_header} -eq 0 ] && [ ${check_hour_change} -eq 0 ] && [ ${check_last_row} -eq 0 ]; then
                    if [ ${debug} -eq 2 ]; then
...
_import_file_add() {
    ...
    line_add=""
    # Since the original line starts with a delimiter, we start at 1.
    for LOOP in $(seq 1 ${columns}); do
    ...
    for LOOP in $(seq ${csv_number_of_split_files}); do
        ...
        if [ ${LOOP} -ne ${csv_file_with_value} ]; then
            line_extracts=$(cat "export-split-${LOOP}.csv" | grep ",${datetime_result}")

            # There can be several rows with same datetime.
            while read line_tmp
            do
                # Split into array.
                IFS=','
                line_tmp_array=(${line_tmp})
                unset IFS

                line_add=""
                # Since the original line starts with a delimiter, we start at 1.
                for LOOP2 in $(seq 1 ${columns}); do
                    value_loop=${line_tmp_array[${LOOP2}]}
